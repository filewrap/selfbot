...
async def cmd_help(_app: TelegramClient, bot: TelegramClient, update, _args: str, _replied: Message):
    commands = ", ".join(f"?{k}" for k in COMMANDS.keys())
    text = f"**Available commands:**\n{commands}"
    from telethon import Button
    buttons = [[Button.inline("Delete", b"0"), Button.inline("Hide", b"0")]]
    await bot.send_message(update.message.peer_id, text, buttons=buttons)
    # delete the original command message
    await bot.delete_messages(update.message.peer_id, [update.message.id])
    return None
...
async def cmd_info(app: TelegramClient, bot: TelegramClient, update, args: str, replied: Message):
    await bot.delete_messages(update.message.peer_id, [update.message.id])
    try:
        # invoke inline via user client
        bot_entity = await app.get_input_entity(app.user_id)
        results = await app.inline_query(bot_entity, "info")
        if results:
